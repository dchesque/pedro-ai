...
The application uses Clerk webhooks to synchronize user data between Clerk and the local database. This ensures that user information is consistent across both systems.
...
export async function POST(request: Request) {
  try {
    ...
    switch (event.type) {
      ...
      default:
        // Optional: log unhandled events for monitoring
...
async function handleUserCreated(event: WebhookEvent) {
  ...
  // Create the user and initialize their credit balance
  await db.user.create({
    data: {
      ...
      creditBalance: {
        create: {
          clerkUserId: id,
          creditsRemaining: 0, // Initialize with 0 credits
        }
      }
    }
  });
...
async function handleUserDeleted(event: WebhookEvent) {
  // Ensure the user ID exists before attempting to delete
  if (!event.data.id) {
    return;
  }
  // Delete the user and all related data (cascading delete should be configured in Prisma schema)
