...
- Database operations
- External service integrations (Asaas, etc.)
...
### Admin Settings & Plan Management
- **Custos de Features**: O administrador atualiza os custos em `/admin/settings`, que chama `PUT /api/admin/settings` para salvar as configurações na tabela `AdminSettings`.
- **Gerenciamento de Planos**: Os planos são criados e gerenciados manualmente em `/admin/settings/plans`. As operações (CRUD) são feitas através dos endpoints `/api/admin/plans`.
- **Consumo na UI**: A UI lê os custos das features de `GET /api/credits/settings` para exibir informações e habilitar/desabilitar ações.
