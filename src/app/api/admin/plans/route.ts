...
function validateAsaasMinimumValue(cents: number | null): { valid: boolean; error?: string } {
...
async function handleAdminPlansPost(req: Request) {
  ...
  try {
    ...
    const monthlyValidation = validateAsaasMinimumValue(monthlyPriceCents)
    if (!monthlyValidation.valid) {
      return NextResponse.json({ error: `Preço mensal: ${monthlyValidation.error}` }, { status: 400 })
    }

    const yearlyValidation = validateAsaasMinimumValue(yearlyPriceCents)
    ...
    if (currency !== 'brl') {
      return NextResponse.json({
        error: 'Apenas a moeda BRL (Real brasileiro) é aceita. Asaas não suporta pagamentos em outras moedas.'
      }, { status: 400 })
    }
